...
const {ZeroAddress, Zero} = require('@animoca/ethereum-contracts-core/src/constants');
...
async function getInstance(token, account, totalSupply, config) {
  ...
  return await F1DTCrateKey.new(token.symbol, token.name, token.uri, account || TOKEN_HOLDER, totalSupply || token.totalSupply, config);
}
...
function tokenConstructorChecks(token) {
  it('should revert with invalid symbol', async function () {
    await expectRevert(getInstance({...token, symbol: ''}), 'F1DTCrateKey: invalid symbol');
  });
  it('should revert with invalid name', async function () {
    await expectRevert(getInstance({...token, name: ''}), 'F1DTCrateKey: invalid name');
  });
  it('should revert with invalid holder', async function () {
    await expectRevert(getInstance(token, ZeroAddress), 'F1DTCrateKey: invalid holder');
  });
  it('should revert with a zero supply', async function () {
    await expectRevert(getInstance(token, deployer, Zero), 'F1DTCrateKey: invalid initial supply');
  });
    ...
    describe('Burn Operation', function () {
      describe('F1DT.CCK', function () {
        it('should fail due to invalid onwer', async function () {
          await expectRevert(this.f1dtCck.burn(TOKENS.F1DT_CCK.totalSupply, {from: operator}), 'Ownable: caller is not the owner');
        });
        it('should fail due to zero amount', async function () {
          await expectRevert(this.f1dtCck.burn(Zero, {from: deployer}), 'F1DTCrateKey: invalid amount');
        });
        it('should fail due to invalid amount', async function () {
          await expectRevert(this.f1dtCck.burn(toWei('100000'), {from: deployer}), 'ERC20: burn amount exceeds balance');
        });
      ...
      describe('F1DT.RCK', function () {
        it('should fail due to invalid onwer', async function () {
          await expectRevert(this.f1dtRck.burn(TOKENS.F1DT_RCK.totalSupply, {from: operator}), 'Ownable: caller is not the owner');
        });
        it('should fail due to zero amount', async function () {
          await expectRevert(this.f1dtRck.burn(Zero, {from: deployer}), 'F1DTCrateKey: invalid amount');
        });
        it('should fail due to invalid amount', async function () {
          await expectRevert(this.f1dtRck.burn(toWei('100000'), {from: deployer}), 'ERC20: burn amount exceeds balance');
        });
        it('should burn the tokens', async function () {
      ...
      describe('F1DT.ECK', function () {
        it('should fail due to invalid onwer', async function () {
          await expectRevert(this.f1dtEck.burn(TOKENS.F1DT_ECK.totalSupply, {from: operator}), 'Ownable: caller is not the owner');
        });
        it('should fail due to zero amount', async function () {
          await expectRevert(this.f1dtEck.burn(Zero, {from: deployer}), 'F1DTCrateKey: invalid amount');
        });
        it('should fail due to invalid amount', async function () {
          await expectRevert(this.f1dtEck.burn(toWei('100000'), {from: deployer}), 'ERC20: burn amount exceeds balance');
        });
        it('should burn the tokens', async function () {
      ...
      describe('F1DT.LCK', function () {
        it('should fail due to invalid onwer', async function () {
          await expectRevert(this.f1dtLck.burn(TOKENS.F1DT_LCK.totalSupply, {from: operator}), 'Ownable: caller is not the owner');
        });
        it('should fail due to zero amount', async function () {
          await expectRevert(this.f1dtLck.burn(Zero, {from: deployer}), 'F1DTCrateKey: invalid amount');
        });
        it('should fail due to invalid amount', async function () {
          await expectRevert(this.f1dtLck.burn(toWei('100000'), {from: deployer}), 'ERC20: burn amount exceeds balance');
        });
        it('should burn the tokens', async function () {
