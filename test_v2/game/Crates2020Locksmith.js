...
const Locksmith = artifacts.require('Crates2020Locksmith');
...
describe('Crates2020Locksmith', function () {
  ...
  function createPayloadHash(holder, crateTier, nonce) {
    const payload = web3.eth.abi.encodeParameters(['address', 'uint256', 'uint256'], [holder, `${crateTier}`, nonce.toString()]);
  ...
  describe('insertKeys()', function () {
    ...
    for (const [tier, crate] of Object.entries(crates)) {
      it(`[${tier}] should revert with an incorrect quantity`, async function () {
        await expectRevert(this.locksmith.insertKeys(crate.tier, maxQuantity + 1, '0x', {from: deployer}), 'Locksmith: above max quantity');
      });

      it(`[${tier}] should revert if the signer key is not set`, async function () {
        await expectRevert(this.locksmith.insertKeys(crate.tier, 1, '0x', {from: deployer}), 'Locksmith: signer key not set');
      });
      ...
      for (let quantity = 1; quantity <= maxQuantity; quantity++) {
        it(`[${tier}] should revert if the sigature is from the wrong signer, qty=${quantity}`, async function () {
          ...
          await expectRevert(this.locksmith.insertKeys(crate.tier, quantity, this.signature, {from: deployer}), 'Locksmith: invalid signature');
        });
        ...
        describe(`[${tier}] on success, qty=${quantity}`, function () {
          ...
          it(`[${tier}] should fail if trying to use the same signature again, qty=${quantity}`, async function () {
            await expectRevert(this.locksmith.insertKeys(crate.tier, 1, this.signature, {from: holder}), 'Locksmith: invalid signature');
          });
          ...
          it(`[${tier}] should cost less gas the second time, qty=${quantity}`, async function () {
            ...
            console.log(`Opening ${quantity} ${tier} crates: Bootstrap gas cost: ${this.gasUsed}, later: ${newGasUsed}`);
