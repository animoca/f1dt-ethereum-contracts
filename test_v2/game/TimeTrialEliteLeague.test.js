...
const {ZeroAddress, EthAddress} = require('@animoca/ethereum-contracts-core').constants;
...
describe('TimeTrialEliteLeague', function () {
  ...
  describe('enterTier(tierId, deposit)', function () {
    ...
    describe('when not paused', function () {
      it('should revert if tier does not exist', async function () {
        await expectRevert(this.timeTrialEliteLeague.enterTier(fromAscii('D'), 10, {from: participant}), 'Leagues: tier not found');
      });

      it('should revert if insufficient deposit', async function () {
        await expectRevert(this.timeTrialEliteLeague.enterTier(fromAscii('A'), 100, {from: participant}), 'Leagues: insufficient amount');
      });
      ...
      it('should revert if already participant', async function () {
        ...
        await expectRevert(this.timeTrialEliteLeague.enterTier(fromAscii('A'), toWei('100000'), {from: participant}), 'Leagues: already participant');
      });
    ...
    describe('when paused', function () {
      ...
      it('should revert if tier does not exist (paused)', async function () {
        await expectRevert(this.timeTrialEliteLeague.enterTier(fromAscii('D'), 10, {from: participant}), 'Pausable: paused');
      });

      it('should revert if insufficient deposit (paused)', async function () {
        await expectRevert(this.timeTrialEliteLeague.enterTier(fromAscii('A'), 100, {from: participant}), 'Pausable: paused');
      });

      it('should revert if not enough REVV in wallet (paused)', async function () {
        await this.revv.transfer(deployer, toWei('999990'), {from: participant}); // remaining 10 REVV
        await expectRevert(this.timeTrialEliteLeague.enterTier(fromAscii('A'), toWei('100000'), {from: participant}), 'Pausable: paused');
      });
  ...
  describe('increaseDeposit(tierId, amount)', function () {
    ...
    describe('when not paused', function () {
      it('should revert if tier does not exist', async function () {
        await expectRevert(this.timeTrialEliteLeague.increaseDeposit(fromAscii('D'), 10, {from: participant}), 'Leagues: tier not found');
      });

      it('should revert if user is not in tier', async function () {
        await expectRevert(this.timeTrialEliteLeague.increaseDeposit(fromAscii('B'), 10, {from: participant}), 'Leagues: non participant');
      });
    ...
    describe('when paused', function () {
      ...
      it('should revert if tier does not exist (paused)', async function () {
        await expectRevert(this.timeTrialEliteLeague.increaseDeposit(fromAscii('D'), 10, {from: participant}), 'Pausable: paused');
      });

      it('should revert if user is not in tier (paused)', async function () {
        await expectRevert(this.timeTrialEliteLeague.increaseDeposit(fromAscii('B'), 10, {from: participant}), 'Pausable: paused');
      });

      it('should revert with correct parameters (paused)', async function () {
        await expectRevert(this.timeTrialEliteLeague.increaseDeposit(fromAscii('A'), toWei('1000'), {from: participant}), 'Pausable: paused');
      });
    });
  });
  ...
  describe('exitTier(tierId)', function () {
    ...
    describe('when not paused', function () {
      it('should revert if the tier does not exist', async function () {
        await expectRevert(this.timeTrialEliteLeague.exitTier(fromAscii('D'), {from: participant}), 'Leagues: non-participant');
      });

      it('should revert if the participant is not in the tier', async function () {
        await expectRevert(this.timeTrialEliteLeague.exitTier(fromAscii('B'), {from: participant}), 'Leagues: non-participant');
      });

      it('should revert if the participant is still time locked', async function () {
        await expectRevert(this.timeTrialEliteLeague.exitTier(fromAscii('A'), {from: participant}), 'Leagues: time-locked');
      });
