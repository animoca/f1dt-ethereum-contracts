...
// const {ZeroAddress, Zero, One, Two, Three, Four, Five, ZeroBytes32} = require('@animoca/ethereum-contracts-core').constants;
...
// const DeltaTimeStaking = artifacts.require('DeltaTimeStaking');
...
// const tokens = cars.concat(drivers).map((token) => {
...
// const invalidTokens = tyres.map((token) => {
//   token.id = createTokenId(token, true);
...
//   describe(
//     'constructor(CycleLengthInSeconds, ' +
//       'PeriodLengthInCycles, inventoryContract, revvContract, weights, rarities, revvEscrowingWeightCoefficient)',
//     function () {
...
//         await expectRevert(this.staking.unstakeNft(tokens[0].id, {from: staker}), 'NftStaking: not staked for owner');
//       });
...
//         const receipt = await this.staking.batchUnstakeNfts(
//           params.map((token) => token.id),
//           {from: staker}
//         );
