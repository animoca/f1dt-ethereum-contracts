...
const {ZeroAddress, Zero, One, Two, Five, MaxUInt256} = require('@animoca/ethereum-contracts-core').constants;
...
const Crates = artifacts.require('Crates2020Mock');
  ...
  describe('openCrates()', function () {
    ...
    for (const [tier, crate] of Object.entries(crates)) {
      it(`[${tier}] should revert with zero quantity`, async function () {
        await expectRevert(this.crates.openCrates(crate.tier, 0, seed, {from: deployer}), 'Crates: zero quantity');
      });
      ...
      for (let quantity = 1; quantity <= maxQuantity; quantity++) {
        it(`[${tier}] should revert if the caller does not have a key, qty=${quantity}`, async function () {
          await expectRevert(this.crates.openCrates(crate.tier, quantity, seed, {from: deployer}), 'ERC20: transfer amount exceeds balance');
        });

        it(`[${tier}] should revert if the holder did not set allowance to the contract, qty=${quantity}`, async function () {
          await expectRevert(this.crates.openCrates(crate.tier, quantity, seed, {from: holder}), 'ERC20: transfer amount exceeds allowance');
        });
        ...
        it(`[${tier}] should revert if the crates is not the crate key contract owner, qty=${quantity}`, async function () {
          ...
          await expectRevert(this.crates.openCrates(crate.tier, quantity, seed, {from: holder}), 'Ownable: caller is not the owner');
        });
        ...
        it(`[${tier}] should revert if the crates is not an inventory minter, qty=${quantity}`, async function () {
          ...
          await this.crateKeys[tier].approve(this.crates.address, MaxUInt256, {from: holder});
          await expectRevert(this.crates.openCrates(crate.tier, quantity, seed, {from: holder}), 'MinterRole: caller does not have the Minter role');
        });
  ...
  describe('transferCrateKeyOwnership()', function () {
    ...
    for (const [tier, crate] of Object.entries(crates)) {
      it(`[${tier}] should revert if not called by the owner of this contract`, async function () {
        await expectRevert(this.crates.transferCrateKeyOwnership(crate.tier, deployer, {from: holder}), 'Ownable: caller is not the owner');
      });

      it(`[${tier}] should revert if the crate key is not owned by this contract`, async function () {
        await expectRevert(this.crates.transferCrateKeyOwnership(crate.tier, deployer, {from: deployer}), 'Ownable: caller is not the owner');
      });
