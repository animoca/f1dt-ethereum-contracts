...
const {
  ...
  Five,
  ZeroBytes32,
} = require('@animoca/ethereum-contracts-core').constants;
...
const defectiveTokens = [createTokenId({counter: 1}, false), createTokenId({counter: 2}, false), createTokenId({counter: 3}, false)];

const replacementTokens = [createTokenId({counter: 4}, false), createTokenId({counter: 5}, false), createTokenId({counter: 6}, false)];

describe('NFTRepairCentre', function () {
  let Bytes, DeltaTimeInventory;
  ...
  describe('addTokensToRepair([defectiveTokens], [replacementTokens])', function () {
    ...
    it('should revert with inconsistent array lengths', async function () {
      await expectRevert(this.repairCentre.addTokensToRepair(['0x1'], ['0xa', '0xb'], {from: deployer}), 'RepairCentre: wrong lengths');
    });

    it('should revert if not called by the owner', async function () {
      await expectRevert(this.repairCentre.addTokensToRepair(['0x1'], ['0xa'], {from: payout}), 'Ownable: caller is not the owner');
    });
    ...
    it('should revert if the REVV transfer fails', async function () {
      ...
      // no REVV approval
      await expectRevert.unspecified(this.repairCentre.addTokensToRepair(defectiveTokens, replacementTokens, {from: deployer}));
    });
  ...
  describe('repair token with ERC1155 safeTransferFrom', function () {
    beforeEach(async function () {
      ...
      await this.inventory.batchMint([owner, owner, owner], defectiveTokens, [ZeroBytes32, ZeroBytes32, ZeroBytes32], [1, 1, 1], true, {
        from: deployer,
      });
    });
    ...
    it('should revert if the transfer is coming from another NFT contract', async function () {
      ...
      const InventoryClone = artifacts.require('artifacts_v0:DeltaTimeInventory');
      const bytes = await Bytes.new({from: deployer});
      await InventoryClone.link(bytes);
      const inventory = await InventoryClone.new(this.revv.address, graveyard, {from: deployer});
      await inventory.batchMint([owner, owner, owner], defectiveTokens, [ZeroBytes32, ZeroBytes32, ZeroBytes32], [1, 1, 1], true, {
        from: deployer,
      });

      await expectRevert(
        inventory.methods['safeTransferFrom(address,address,uint256,uint256,bytes)'](owner, this.repairCentre.address, defectiveTokens[0], 1, '0x0', {
          from: owner,
        }),
    ...
    it('should revert if the token is not defective', async function () {
      ...
      await expectRevert(
        this.inventory.methods['safeTransferFrom(address,address,uint256,uint256,bytes)'](owner, this.repairCentre.address, validToken, 1, '0x0', {
          from: owner,
        }),
    ...
    it('should replace the defective token', async function () {
      for (let i = 0; i < defectiveTokens.length; i++) {
        ...
        const revvTransferEvent = web3.eth.abi.decodeLog(revvTransferEventABI.inputs, revvTransferLog.data, revvTransferLog.topics.slice(1));
