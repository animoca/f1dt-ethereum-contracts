...
const shouldHaveNextClaim = function (staker, params) {
  it(
    `nextClaim for staker [${staker}] should have period=${params.period}, ` +
      `globalSnapshotIndex=${params.globalSnapshotIndex} and stakerSnapshotIndex=${params.stakerSnapshotIndex}`,
    async function () {
...
const shouldHaveStakerHistoryLength = function (staker, count) {
  it(`should have staker history length: ${count}`, async function () {
    if (count == 0) {
      await expectRevert(this.stakingContract.lastStakerSnapshotIndex(staker), 'NftStaking: empty staker history');
    } else {
