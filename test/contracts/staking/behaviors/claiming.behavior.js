...
} = require('@animoca/ethereum-contracts-core/test/utils/weiPrecision');
...
const shouldClaimRewards = function (staker, maxPeriods, params) {
  it(
    `[claimRewards] ${params.amount} tokens over ${params.periods} periods` + ` (max=${maxPeriods}) starting at ${params.startPeriod}, by ${staker}`,
    async function () {
      params.amount = toWei(params.amount);
