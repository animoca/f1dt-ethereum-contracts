...
const preconditionsScenario = function () {
  ...
  context('Staking contract', function () {
    ...
    if (this.debug) {
      it('debugs the current state', async function () {
        await debugCurrentState.bind(this)();
      });
    }
  });
  ...
  context('NFT Assets Inventory contract', function () {
    ...
    it('should have minted an Epic car token (1) for the staker', async function () {
      const tokenId = this.stakerTokens[staker][1];
      const rarity = TokenHelper.getRarity(tokenId);
      rarity.should.be.equal(TokenHelper.Rarities.Epic);
    });

    it('should have minted an Apex car token (2) for the staker', async function () {
      const tokenId = this.stakerTokens[staker][2];
      const rarity = TokenHelper.getRarity(tokenId);
      rarity.should.be.equal(TokenHelper.Rarities.Legendary);
    });

    it('should have minted an Apex car token (3) for the staker', async function () {
