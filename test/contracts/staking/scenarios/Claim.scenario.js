...
const {shouldStakeNft, shouldUnstakeNft, shouldClaimRewards, shouldTimeWarpBy, initialiseDebug, mintStakerTokens} = require('../behaviors');
...
const claimScenario = function () {
  ...
  describe('when estimating after disabling', function () {
    beforeEach(async function () {
      await this.stakingContract.disable({from: creator});
    });
    it('reverts when estimating after disabling', async function () {
      await expectRevert(this.stakingContract.estimateRewards(99, {from: staker}), 'NftStaking: contract is not enabled');
    });
  });

  describe('when claiming after disabling', function () {
    beforeEach(async function () {
      await this.stakingContract.disable({from: creator});
    });
    it('reverts when estimating after disabling', async function () {
      await expectRevert(this.stakingContract.claimRewards(99, {from: staker}), 'NftStaking: contract is not enabled');
    });
  });
};
