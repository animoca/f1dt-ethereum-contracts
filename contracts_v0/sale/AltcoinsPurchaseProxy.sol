...
contract AltcoinsPurchaseProxy is Ownable {
    ...
    function purchaseFor(
        ...
        (vars.altcoinConversionRate, vars.altcoinPrice, vars.altcoinPriceWithoutDiscount) = getPrice(
            lotId,
            quantity,
            ERC20(address(altcoinAddress)),
            destination
        );
        ...
        (vars.stablecoinConversionRate, vars.stablecoinPrice, vars.stablecoinPriceWithoutDiscount) = getPrice(
            lotId,
            quantity,
            _saleContract._stableCoinAddress(),
            destination
        );
        ...
        emit PurchasedViaProxy(destination, msg.sender, lotId, quantity, vars.altcoinPrice, address(altcoinAddress), referrer);
    }
