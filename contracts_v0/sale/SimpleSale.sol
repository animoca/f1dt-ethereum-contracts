...
contract SimpleSale is Ownable, GSNRecipient {
    ...
    enum ErrorCodes {RESTRICTED_METHOD, INSUFFICIENT_BALANCE}
    ...
    event Purchased(string purchaseId, address paymentToken, uint256 price, uint256 quantity, address destination, address operator);

    event PriceUpdated(string purchaseId, uint256 ethPrice, uint256 erc20Price);
    ...
    function purchaseFor(
        ...
        if (paymentToken == ETH_ADDRESS) {
            ...
            uint256 totalPrice = price.ethPrice.mul(quantity);
            ...
            uint256 totalPrice = price.erc20Price.mul(quantity);
    ...
    ) external view returns (uint256, bytes memory mem) {
