...
contract FixedSupplyCratesSale is ReferrableSale, Pausable, KyberAdapter, ICrateOpenEmitter {
    ...
    event LotCreated(uint256 lotId, uint256 supply, uint256 price, string uri, ERC20 crateToken);

    event LotPriceUpdated(uint256 lotId, uint256 price);
    ...
    ) public KyberAdapter(kyberProxy) {
    ...
    function _nthPurchaseDiscount(
        uint256 lotPrice,
        uint256 quantity,
        uint256 cratesPurchased
    ) private view returns (uint256) {
        uint256 discountsApplied = cratesPurchased / MULTI_PURCHASE_DISCOUNT_STEP;
        uint256 discountsToApply = (cratesPurchased + quantity) / MULTI_PURCHASE_DISCOUNT_STEP - discountsApplied;

        return lotPrice.mul(discountsToApply).mul(_multiPurchaseDiscount).div(PERCENT_PRECISION);
    }

    function _getPriceWithDiscounts(
        Lot memory lot,
        uint256 quantity,
        uint256 cratesPurchased
    ) private view returns (uint256 price, uint256 discount) {
    ...
    function purchaseFor(
    ...
    ) external payable whenNotPaused whenStarted {
        require(quantity > 0);
        ...
        if (referrer != address(0)) {
            ...
            if (sendReferral) {
                vars.referralReward = vars.tokensReceived.mul(Math.max(_customReferralPercentages[referrer], _defaultReferralPercentage)).div(
                    PERCENT_PRECISION
                );
        ...
        )
    {
        ...
        (uint256 totalPrice, ) = _getPriceWithDiscounts(_lots[lotId], quantity, _cratesPurchased[destination][lotId]);

        (, uint256 tokenAmount) = _convertToken(_stableCoinAddress, totalPrice, tokenAddress);
