...
contract DeltaTimeInventory is PausableInventory, ERC20Fees, MinterRole {
     ...
     */
    constructor(address gasTokenAddress, address payoutWallet) public PausableInventory(32) ERC20Fees(gasTokenAddress, payoutWallet) {
        _ipfsMigrated = false;
    }
     ...
     */
    function createCollection(uint256 collectionId, bytes32 byteUri) external onlyMinter {
    ...
    function batchMint(
        ...
        require(ids.length == to.length && ids.length == uris.length && ids.length == values.length);
    ...
    function _mintNonFungible(
        ...
        if (safe) {
            require(_checkERC1155AndCallSafeTransfer(_msgSender(), address(0x0), to, id, 1, "", false, false), "failCheck"); // solium-disable-line error-reason
        }
    }
    ...
    function _mintFungible(
        ...
        require(_checkERC1155AndCallSafeTransfer(_msgSender(), address(0x0), to, collection, value, "", false, false), "failCheck"); // solium-disable-line error-reason
    }
